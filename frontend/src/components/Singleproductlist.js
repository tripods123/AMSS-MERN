...
function Singleproductlist(props){
    ...
    if(data.length>0){
        return(
            <div>
                {data.map(({_id,name,price,image,shop,brand})=>(
                    <Link to={"/productdisplay/"+_id} key={_id} style={{'textDecoration':'none'}}>
                        <div className='col align-items-start' key={_id}>
                            <div className="card">
                                <div className="row g-0">
                                    <div className="col-md-4">
                                        <img className="img-thumbnail" src={image[0]} alt={name}/>
                                    </div>
                                    <div className="col-md-8">
                                        <div className="card-body">
                                            <h5 className="card-title text-start display-6" style={{'textDecoration':'none'}}>{name}</h5>
